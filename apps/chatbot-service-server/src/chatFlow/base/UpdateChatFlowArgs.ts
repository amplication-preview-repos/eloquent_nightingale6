/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ChatFlowWhereUniqueInput } from "./ChatFlowWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ChatFlowUpdateInput } from "./ChatFlowUpdateInput";

@ArgsType()
class UpdateChatFlowArgs {
  @ApiProperty({
    required: true,
    type: () => ChatFlowWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ChatFlowWhereUniqueInput)
  @Field(() => ChatFlowWhereUniqueInput, { nullable: false })
  where!: ChatFlowWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ChatFlowUpdateInput,
  })
  @ValidateNested()
  @Type(() => ChatFlowUpdateInput)
  @Field(() => ChatFlowUpdateInput, { nullable: false })
  data!: ChatFlowUpdateInput;
}

export { UpdateChatFlowArgs as UpdateChatFlowArgs };
